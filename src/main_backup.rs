...
/// Function to run commit analysis with default settings
async fn run_commit_analysis(count: usize, format: &str, with_ai: bool) -> Result<()> {
    println!("🔍 Analyzing last {} commits for inconsistencies and typos...", count);
    
    let analyzer = CommitAnalyzer::new()?;
    let analyses = analyzer.analyze_last_commits(count).await?;
        ...
        if !is_recent_commit(&recent_commits, &commit_msg) {
            ...
            let edits = match i {
                ...
                _ => vec![Edit::SearchReplace {
                    ...
                    search: "name = \"nautilus-trader-rig\"".to_string(),
                    replace: "name = \"nautilus-trader-rig\"".to_string(),
                    occurrences: Some(1),
                }],
            };
